...
from .forms import UserForm,UpdateUserForm, MessageImage
...
def home(request):
    ...
    if request.user.is_authenticated:
        ...
        room_messages = Message.objects.filter(
            ...
            house__topic__name__icontains = q,)[0:5 ]
    else:
...
def room(request, pk):
    ...
    if request.method == "POST":
        message = Message.objects.create(
            ...
            body_image = request.user.avatar
        )
...
@login_required(login_url='/login_page')
def messageImage(request, pk):
    user = request.user
    room = Room.objects.get(id=pk)
    form = MessageImage()
    if request.method == 'POST':
        form = MessageImage(request.POST,request.FILES)
        if form.is_valid():
            real = form.save(commit = False)
            real.user= request.user
            real.body = request.POST.get('body')
            real.house = room
            real.save()
            return redirect('room', pk = room.id)
    return render(request , 'main/message-image.html', {'form':form})
